...
async function bulkerSchedule() {
  try {
    ...
    for (let doctor of doctors) {
      ...
      const newSpecialties = getRandomSpecialties(specialtiesFound, 1, 3); // Asignar especialidades al doctor
      doctor.specialties = newSpecialties.map((specialty) => specialty._id); // Relacionar las especialidades

      // Asignar una ciudad aleatoria de Bolivia
      doctor.city = faker.helpers.arrayElement(bolivianCities); // Ciudad aleatoria

      // Decidir si asignar horarios o no (50% de chance)
      ...
      if (assignSchedules) {
        ...
        const scheduleDocs = await Schedule.create(
          schedules.map((schedule) => ({
            doctor: doctor._id, // Relacionar el horario con el doctor
            ...schedule, // Incluir los datos generados del horario
          }))
        );

        // Asignar los horarios al doctor
        doctor.schedule = scheduleDocs.map((schedule) => schedule._id);
      } else {
        doctor.schedule = []; // Si no se asignan horarios, el campo 'schedule' estará vacío
      }

      // Guardar el doctor con las nuevas especialidades, ciudad y horarios
      await doctor.save();
      console.log(
