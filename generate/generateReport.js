...
async function generateReport(req, res) {
  ...
  try {
    ...
    // // Obtener los datos (esto es solo un ejemplo)
    // const availableYears = await getAvailableYears(Model);
    const activeData = await getMonthlyActiveData(Model, year);
    const inactiveData = await getMonthlyInactiveData(Model, year);
    ...
    // *** Aquí es donde colocas el código de Puppeteer ***
    const browser = await puppeteer.launch({
      headless: true, // Ejecutar sin interfaz gráfica
      args: ['--no-sandbox', '--disable-setuid-sandbox'], // Necesario en Gitpod
    });
      ...
      <html>
        ...
        <body>
          ...
          <script>
            const ctx = document.getElementById('myChart').getContext('2d');
            const myChart = new Chart(ctx, {
              type: '${chartType}', // El tipo de gráfico que elijas (ej. 'bar', 'line', etc.)
              data: {
                labels: ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'],
                datasets: [{
                  label: 'Registros Activos',
    ...
    await page.waitForSelector('#myChart'); // Asegurarse de que el gráfico esté completamente cargado
