...
import { isAuthenticated, options, token } from './options.js';
...
// Suponiendo que 'contextMenu' y 'currentElement' ya están definidos en el código

document.addEventListener('contextmenu', (event) => {
  // Si el usuario NO está autenticado, no bloqueamos el menú
  if (!isAuthenticated || !token) {
    return;
  }

  // Bloqueamos el menú predeterminado si el usuario está autenticado
  event.preventDefault();

  currentElement = event.target; // Asignar el elemento sobre el que se hizo clic
  if (currentElement) {
    ...
    // Mostrar el menú contextual en la ubicación del clic
